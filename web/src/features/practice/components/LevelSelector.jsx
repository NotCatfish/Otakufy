...
export default function LevelSelector({ category, currentTheme, engineState }) {
  ...
  return (
    <div className="max-w-[1440px] w-full mx-auto py-32 px-4 md:px-8 text-center relative z-10 font-light text-white/80 flex flex-col items-center">
      <SmoothFade delay={0.1} className="self-start md:self-auto w-full flex justify-start md:justify-center mb-8">
      ...
      <SmoothFade delay={0.1} className="flex flex-col items-center">
        <h1 className="text-6xl md:text-7xl font-bold mb-6 tracking-tight capitalize text-white">
          <RevealText text={lang === 'ja' ? (category === 'vocabulary' ? '単語' : category === 'grammar' ? '文法' : category === 'kanji' ? '漢字' : category === 'reading' ? '読解' : category === 'random' ? 'ランダム' : category) : category} baseDelay={0.2} />
        </h1>
      ...
      <SmoothFade delay={0.4} className="flex flex-wrap justify-center gap-5 w-full max-w-[1440px] mt-12">
        {['N1', 'N2', 'N3', 'N4', 'N5', 'SRS', category === 'random' ? 'Global' : 'Random'].map((lvl, index) => {
          ...
          >
            <span className={`text-3xl font-bold ${currentTheme.color} transition-colors tracking-tight whitespace-nowrap`}>
              <RevealText text={lvl === 'Global' && lang === 'ja' ? '全レベル' : lvl === 'Random' && lang === 'ja' ? 'ランダム' : displayLvl} baseDelay={0.5 + (index * 0.05)} />
            </span>
            <span className={`text-[11px] font-medium uppercase tracking-[0.2em] mt-3 ${currentTheme.color} opacity-80 group-hover:opacity-100 transition-colors whitespace-nowrap`}>
              <RevealText text={lvl === 'SRS' ? (lang === 'ja' ? '間隔反復' : 'Spaced Rep') : lvl.startsWith('N') ? (lang === 'ja' ? 'JLPT レベル' : 'JLPT Level') : (lang === 'ja' ? 'ミックスデッキ' : 'Mixed Deck')} baseDelay={0.5 + (index * 0.05)} />
            </span>
          </button>
        )})}
      </SmoothFade>
